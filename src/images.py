...
def download_image(download_url, save_path):
    ...
    if response.status_code != 200:
        raise Exception(f"Failed to download image from {download_url}: {response.text}")
...
def edit_image(input_path, output_path, first_name, last_name, country):
    ...
    name_font = ImageFont.truetype("utils\\Playfair_Display\\static\\PlayfairDisplay-ExtraBold.ttf", NAME_FONT_SIZE)
    ...
    image_bottom = content_bottom - country_h - COUNTRY_HANDLE_GAP - handle_h - TEXT_TO_IMAGE_GAP
    ...
    user_img.thumbnail((int(image_area_width), int(image_area_height)), Image.Resampling.LANCZOS)
    ...
    draw.text((center_x, name_y), name_text, font=name_font, anchor="mm", fill=TEXT_COLOR)
    
    # Country near bottom
    country_y = content_bottom - handle_h - COUNTRY_HANDLE_GAP - country_h // 2
    draw.text((center_x, country_y), country, font=country_font, anchor="mm", fill=TEXT_COLOR)
    
    # Minerva handle at very bottom (just above border)
    handle_y = content_bottom - handle_h // 2
    draw.text((center_x, handle_y), handle_text, font=handle_font, anchor="mm", fill=TEXT_COLOR)
